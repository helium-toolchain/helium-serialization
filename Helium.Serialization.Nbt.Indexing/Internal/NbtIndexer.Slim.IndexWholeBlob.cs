	...
	public readonly SlimNbtIndexNode IndexWholeTreeSlim()
	{
		...
		while(true)
		{
			...
			if(workingRegister[0] == 0x00)
			{
				...
				// ascertain everything works even if the original data doesn't supply a root compound,
				// which is the case for blobs generated by older minecraft versions.
				if(indexStack.Count == 0 && startOffset == this.Blob.Length - 1)
				{
					break;
				}
			}
